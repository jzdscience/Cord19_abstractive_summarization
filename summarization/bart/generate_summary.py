...
from transformers import BartForConditionalGeneration, BartTokenizer, AutoModelWithLMHead
...
def generate_summaries(
    ...
    #model.save_pretrained("bart_utest_output")
...
def run_generate():
    ...
    print(examples, args.output_path, args.model_name, args.bs, args.device)
    
    generate_summaries(examples, args.output_path, args.model_name, batch_size=args.bs, device=args.device)

#     examples = [" " + x.rstrip() for x in open('cnn_tiny/test.source').readlines()]
    
#     generate_summaries(examples, 'bart_utest_output/generated_title.text', 'facebook/bart-large-cnn')
